...
def verify_basic_auth(credentials: HTTPBasicCredentials = Depends(security)):
    if credentials.username != settings.API1_USERNAME or credentials.password != settings.API1_PASSWORD:
        raise HTTPException(
...
def verify_basic_auth_admin(credentials: HTTPBasicCredentials = Depends(security)):
    if credentials.username != settings.API1_USERNAME or credentials.password != settings.API1_ADM_PASSWORD:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid credentials",
            headers={"WWW-Authenticate": "Basic"},
...
app = FastAPI(
    title=settings.API1_NAME,
    version=settings.API1_VERSION
)
...
def read_root():
    return {"apiName": settings.API1_NAME, "version": settings.API1_VERSION}
